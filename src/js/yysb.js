...
*/
// 这段代码的目的是使得 Recorder 模块能够在 浏览器、Node.js 和 AMD 环境 中正常工作，无论是在前端还是后端都能兼容。它首先通过 factory(window) 将模块代码执行，然后根据不同的模块规范（如 AMD、CommonJS）将模块导出，使得开发者在不同的环境下都能正常加载和使用 Recorder 模块。
(function (factory) {
  // factory 作为一个封装的代码块，允许模块在不同环境下执行。
  factory(window);
  //umd returnExports.js
  // 这段代码检查是否存在 define 函数，并且是否具有 define.amd 属性。define.amd 是 AMD（异步模块定义）规范的标志，表示当前环境支持 AMD 模块。如果是的话，就通过 define 定义模块，返回 Recorder 对象。AMD 是一种模块化的标准，通常在浏览器端使用，如 RequireJS
  if (typeof define == "function" && define.amd) {
    define(function () {   
      return Recorder;
    });
  }
  //   这段代码检查是否存在 module 对象，并且是否具有 exports 属性。如果环境支持 CommonJS（例如 Node.js），那么就可以通过 module.exports 导出 Recorder 模块。CommonJS 是一种模块化标准，广泛用于服务器端的 JavaScript 环境（例如 Node.js
  ...
  "use strict";
  //   在某些需要函数作为参数但不做任何操作的情况下使用。
  var NOOP = function () {};
  //   Recorder 函数接收一个参数 set，并返回通过 new initFn(set) 创建的实例
  var Recorder = function (set) {
    return new initFn(set);
  };
  Recorder.LM = "2023-02-01 18:05";
  var RecTxt = "Recorder";
  // 用于获取用户的媒体设备（如麦克风和摄像头）。
  var getUserMediaTxt = "getUserMedia";
  // 这两个常量很可能与音频采样率相关，音频录制或处理过程中常常会用到采样率，可能是为了统一表示和管理采样率相关的参数。
  var srcSampleRateTxt = "srcSampleRate";
  var sampleRateTxt = "sampleRate";
  // 可能是为了表示与捕获（捕获某些事件或异常）相关的操作。
  var CatchTxt = "catch";
...
*/
// 实现了一个简单的 WAV 音频编码功能。它使用了 Recorder 原型上的 wav 方法来将音频数据 res 编码为 WAV 格式，并通过回调函数 True 返回一个包含音频数据的 Blob 对象。
(function () {
  "use strict";
...
*/
// 这段代码的功能是将PCM音频数据处理、封装成Blob对象，或者将PCM数据转换为WAV格式，以便于在浏览器中播放或进一步处理
(function () {
  "use strict";

  Recorder.prototype.enc_pcm = {
    stable: true, // 表示这个功能是稳定的。
    testmsg:
      "pcm为未封装的原始音频数据，pcm数据文件无法直接播放；支持位数8位、16位（填在比特率里面），采样率取值无限制",
  };
  // 这个方法的功能是处理传入的PCM数据，并将它封装成一个Blob对象。PCM数据是音频的原始数据格式，通常需要进行封装才能播放。代码根据传入的数据位深（8位或16位），将采样数据写入ArrayBuffer中，然后通过Blob返回封装好的音频数据。
  // res是原始的PCM数据（一个包含音频采样值的数组）。
  // True是一个回调函数，用来处理生成的Blob对象，通常用于后续操作（如播放、上传等）。
  // False是一个备用回调函数，当前没有使用。
  // Recorder.prototype.pcm：处理PCM数据并封装成Blob对象（PCM是原始音频数据，无法直接播放）。
  ...
  False(msg)
  **/
  // pcm2wav方法的作用是将PCM音频数据转换为WAV格式，并且可以直接播放。WAV格式比PCM更加常见，且可以直接在浏览器中播放，因此这段代码的目的是将PCM数据转码为WAV格式。
  Recorder.pcm2wav = function (data, True, False) {
    // 1、检查data是否是一个Blob对象。如果是，默认使用16kHz的采样率和16位的位深。
    if (data.slice && data.type != null) {
      //Blob 测试用
      data = { blob: data };
    }
    // 2、确保提供了sampleRate（采样率）和bitRate（位深）。
    ...
    // 3、使用FileReader读取PCM数据的Blob对象，并将数据转换为Int16Array（16位的音频数据）。
    var reader = new FileReader();
    reader.onloadend = function () {
      var pcm;
      if (bitRate == 8) {
      ...
      // 4、调用Recorder对象的mock方法，将PCM数据模拟成WAV格式并进行转换。
      Recorder({
        type: "wav",
        ...
        .stop(function (wavBlob, duration) {
          // 5、最终通过回调函数True返回WAV格式的音频数据（wavBlob），以及音频的持续时间（duration）。
          True(wavBlob, duration);
        }, False);
    };
